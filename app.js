...
// VALIDACIÓN DE ARCHIVOS: Solo imágenes para que no reviente la web
const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        if (file.mimetype.startsWith('image/')) {
            ...
            cb(null, false); // Ignora archivos que no sean imágenes
        }
    }
});

const MONGO_URI = "mongodb+srv://admin:biblio1789@cluster0.5de0hkj.mongodb.net/?appName=Cluster0"; 
mongoose.connect(MONGO_URI).then(() => console.log("Biblioteca Blindada Conectada"));
...
app.use(session({ secret: 'biblio-escudo-total', resave: false, saveUninitialized: false }));

// --- HELPER SUBIDA SEGURA ---
...
app.post('/admin/quitar-concursante/:torneoId', async (req, res) => {
    if (req.session.rol === 'admin') await Torneo.findByIdAndUpdate(req.params.torneoId, { $pull: { participantes: req.body.usuario } });
    res.redirect('/');
});

// --- AJUSTES CON FILTRO ANTI-ERRORES ---
app.post('/ajustes', upload.single('nuevaFoto'), async (req, res) => {
    let updateData = { color: req.body.color };
    if (req.file) { // Solo si Multer confirmó que es una imagen
...
app.get('/', async (req, res) => {
    ...
    res.send(`
        ...
        <body>
            <header><b>BIBLIOTECA SEGURA</b></header>
            <div class="tabs">
            ...
            <div class="container">
                <div id="sec-libros" class="section active">
                    ${esAdmin ? `<div class="card"><h4>Nuevo Libro</h4><form action="/add-libro" method="POST" enctype="multipart/form-data"><input name="titulo" placeholder="Título"><input name="autor" placeholder="Autor"><input type="file" name="portada" accept="image/*"><button style="background:${userLog.color}; color:white;">Añadir</button></form></div>` : ''}
                    ${libros.map(l => `
                        <div class="card">
                            ...
                            ${esAdmin ? `
                                <div style="margin-top:10px;">
                                    <details><summary style="font-size:0.7em; color:blue; cursor:pointer;">Editar</summary>
                                        <form action="/admin/editar/libro/${l._id}" method="POST" style="margin-top:10px;">
                                            <input name="titulo" value="${l.titulo}"><input name="autor" value="${l.autor}">
                                            <button style="background:#2ecc71; color:white;">Guardar</button>
                                        </form>
                                        <form action="/admin/borrar/libro/${l._id}" method="POST"><button class="btn-del" style="width:100%;">Borrar</button></form>
                                    </details>
                                </div>` : ''}
                ...
                <div id="sec-torneos" class="section">
                    ...
                    ${torneos.map(t => `
                        <div class="card">
                            <h3>🏆 ${t.nombre}</h3><p>Fecha: ${t.fecha}</p>
                            ${esAdmin ? `
                                <details><summary style="font-size:0.7em; color:blue; cursor:pointer;">Gestionar</summary>
                                    <h5>Participantes:</h5>
                                    ${t.participantes.map(p => `<div style="display:flex; justify-content:space-between;"><span>${p}</span><form action="/admin/quitar-concursante/${t._id}" method="POST"><input type="hidden" name="usuario" value="${p}"><button class="btn-del">X</button></form></div>`).join('')}
                                    <form action="/admin/borrar/torneo/${t._id}" method="POST"><button class="btn-del" style="width:100%; margin-top:10px;">Borrar Torneo</button></form>
                                </details>` : ''}
                            <form action="/unirse-torneo/${t._id}" method="POST"><button style="background:#3498db; color:white;">Inscribirse</button></form>
                        </div>`).join('')}
                </div>

                <div id="sec-ajustes" class="section">
                    <div class="card" style="text-align:center;">
                        <div style="width:100px; height:100px; background:${userLog.color}; border-radius:50%; margin:0 auto 15px; display:flex; justify-content:center; align-items:center; font-size:40px; color:white; border:4px solid white; overflow:hidden;">${avatarContent}</div>
                        <form action="/ajustes" method="POST" enctype="multipart/form-data">
                            <label>Cambiar foto (Solo imágenes):</label>
                        ...
                        <a href="/salir" style="color:red;">Salir</a>
                    </div>
                </div>
...
app.listen(PORT, () => console.log('Servidor protegido contra archivos corruptos'));
